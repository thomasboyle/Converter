...
def convert_video_to_avif_under_size(
    ...
    width, height = calculate_target_resolution(orig_width, orig_height, max_bytes, duration, fps, "avif")

    crf_candidates = [28, 32, 36, 40, 44, 48, 52]
    scale_step = 0.86
    ...
    while True:
        for crf in crf_candidates:
            if progress_cb:
                progress_cb({
                    "phase": "encode",
                    "message": f"Encoding AVIF... {width}x{height} @ {fps}fps, CRF {crf}",
                })
            result = encode_attempt(width, height, crf)
            if result.returncode != 0:
                stderr_text = result.stderr.decode('utf-8', errors='replace') if result.stderr else ""
                stderr_tail = "\n".join(stderr_text.strip().splitlines()[-15:])
                raise ConversionError(f"AVIF encoding failed: {stderr_tail or 'Unknown error'}")

            final_size = os.path.getsize(output_avif_path)
            if final_size <= max_bytes:
                ...
                return output_avif_path, params
