...
def convert_video_to_mp4_under_size(
    ...
    width, height = calculate_target_resolution(orig_width, orig_height, max_bytes, duration, fps, "mp4")
    ...
    while True:
        for crf in crf_candidates:
            if progress_cb:
                progress_cb({
                    "phase": "encode",
                    "message": f"Encoding MP4... {width}x{height} @ {fps}fps, CRF {crf}",
                })
            result = encode_attempt(width, height, crf)
            if result.returncode != 0:
                stderr_text = result.stderr.decode('utf-8', errors='replace') if result.stderr else ""
                stderr_tail = "\n".join(stderr_text.strip().splitlines()[-15:])
                raise ConversionError(f"MP4 encoding failed: {stderr_tail or 'Unknown error'}")
